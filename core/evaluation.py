...
class Trainer():
    def __init__(self,args):
        ...
        # model_weight = torch.load('./pretrained_weights/FlowEstimator.pth')
        model_weight = torch.load('./pretrained_weights/FlowEstimator.pth')
        ...
        pretrained_model = '{}'.format('./pretrained_weights/ECN.pth')
        ...
        data = torch.load('./pretrained_weights/LTN.pth')
        ...
        data = torch.load('./pretrained_weights/fuseformer.pth')
    ...
    def eval(self):
        ...
        for v in range(len(video_name_list)):
            ...
            # set save path
            if self.args.davis:
                save_path = os.path.join(self.args.save_dir,  data_name, test_data_name)
            else:
                if self.args.mask_mode == 0 :
                    save_path = os.path.join(self.args.save_dir,  data_name +  '_moving/',  test_data_name)
                else:
                    save_path = os.path.join(self.args.save_dir,  data_name + '_stationary/',  test_data_name)
            os.makedirs(save_path, exist_ok=True)

            
            # Read frames and correponding masks
            if self.args.davis:
                frames, masks, flow_masks = self.prepare_data(frame_path, mask_path, test_data_name)
            else:
                frames, masks, flow_masks = self.prepare_data(frame_path, mask_path, test_data_name, stationary_mask_path)
            ...
            for idx in range(T):
                ...
                if self.args.save_img:
                    utils.save_img((output+1)/2, '{}/{:05}.png'.format(save_path, idx))
            ...
            if self.args.davis:
                ...
                print('{}-{} : psnr-{:.4f} ssim-{:.4f}, lpips-{:.4f}'.format(str(v), test_data_name, v_psnrs, v_ssims, v_lpips))
