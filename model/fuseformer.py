...
class InpaintGenerator(BaseNetwork):
    def __init__(self, stack_num=8, init_weights=True):
        ...
        for i, d in enumerate(kernel_size):
            n_vecs *= int((output_size[i] + 2 * padding[i] - (d - 1) - 1) / stride[i] + 1) # 사실상 patch 개수를 의미
        for _ in range(stack_num):
            blocks.append(TransformerBlock(hidden=hidden, num_head=num_head, dropout=dropout, n_vecs=n_vecs,
                                           t2t_params=t2t_params))
        self.transformer = nn.Sequential(*blocks)
        self.ss = SoftSplit(channel // 2, hidden, kernel_size, stride, padding, dropout=dropout) # 이게 사실상 image to patch

        self.add_pos_emb = AddPosEmb(n_vecs, hidden) # poisitional embedding

        self.sc = SoftComp(channel // 2, hidden, output_size, kernel_size, stride, padding) # 다시 image로 변형 
